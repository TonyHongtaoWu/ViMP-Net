...
class PairedRandomCrop:
    ...
    def __call__(self, results):
        ...
        results['mask'] = [
            v[top_gt:top_gt + self.gt_patch_size,
              left_gt:left_gt + self.gt_patch_size, ...] for v in results['mask']
        ]
        results['drop'] = [
            v[top_gt:top_gt + self.gt_patch_size,
              left_gt:left_gt + self.gt_patch_size, ...] for v in results['drop']
        ]
        results['streak'] = [
            v[top_gt:top_gt + self.gt_patch_size,
              left_gt:left_gt + self.gt_patch_size, ...] for v in results['streak']
        ]
        ...
        if not gt_is_list:
            results['drop'] = results['drop'][0]
        if not gt_is_list:
            results['streak'] = results['streak'][0]
