...
class GenerateFrameIndiceswithPadding:
    ...
    def __call__(self, results):
        ...
        mask_path_root = results['mask_path']
        ...
        mask_paths = [osp.join(mask_path_root, clip_name, f'{frame_name}.png')]
        drop_paths = [osp.join(drop_path_root, clip_name, f'{frame_name}.png')]
        streak_paths = [osp.join(streak_path_root, clip_name, f'{frame_name}.png')]



        results['lq_path'] = lq_paths
        results['gt_path'] = gt_paths
        results['mask_path'] = mask_paths
        results['drop_path'] = drop_paths
        results['streak_path'] = streak_paths
...
class GenerateFrameIndices:
    ...
    def __call__(self, results):
        ...
        mask_path = [osp.join(mask_path_root, clip_name, f'{frame_name}.png')]
        drop_path = [osp.join(drop_path_root, clip_name, f'{frame_name}.png')]

        streak_path = [osp.join(streak_path_root, clip_name, f'{frame_name}.png')]

        results['lq_path'] = lq_path
        results['gt_path'] = gt_path
        results['mask_path'] = mask_path
        results['drop_path'] = drop_path

        results['streak_path'] = streak_path
...
class GenerateSegmentIndices:
    ...
    def __call__(self, results):
        ...
        mask_path = [
            osp.join(mask_path_root, clip_name, f'{v:08d}.png')
            for v in neighbor_list
        ]
        drop_path = [
            osp.join(drop_path_root, clip_name, f'{v:08d}.png')
            for v in neighbor_list
        ]

        streak_path = [
            osp.join(streak_path_root, clip_name, f'{v:08d}.png')
            for v in neighbor_list
        ]
